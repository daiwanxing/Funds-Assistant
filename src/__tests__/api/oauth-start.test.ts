...
describe("OAuth start helpers", () => {
  ...
  it("redirects supported providers to the Supabase authorization URL with a clean callback URL", async () => {
    const { default: handler } = await import("../../../api/auth/oauth/start.ts");
    const res = createResponse();
  ...
  it("prefers APP_URL over the current request origin when building the OAuth callback", async () => {
    const { default: handler } = await import("../../../api/auth/oauth/start.ts");
    const res = createResponse();
